...
async function startSentinel() {
    console.clear();
    console.log(chalk.magentaBright.bold(`\n âš¡ VERITAS_PROTOCOL // AGENT_ACTIVE`));
    console.log(chalk.cyan(` í¿—ï¸  PROJECT_ID:  ${projectName}`));
    console.log(chalk.cyan(` í¶”  BUILD_ID:    ${BUILD_ID}`));
    ...
    project.stderr.on('data', async (data) => {
        ...
        if (errorMsg.includes('ERR_') || errorMsg.includes('ReferenceError') || errorMsg.includes('FATAL')) {
            console.log(chalk.bgRed.white.bold('\n íº¨ CRITICAL_SECTOR_FAILURE_DETECTED '));
